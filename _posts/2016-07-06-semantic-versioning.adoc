---
layout: post
title:  "Semantic versioning with Gradle and Nebula"
date:   2016-07-06 19:27:29 +0200
categories: oss
tags: git nebula netflix jenkins semantic-versioning gradle
...
This is not about general guidelines or so, but merely a simple tutorial on how to put all the available bricks together in order to have a OSS compliant project.
It contains useful snippets you can use (as is, or variants of them) in your OSS project.
There's also quite a bunch of useful links at the end of the post that I encourage you to read.
...
First, let's start by setting up the gradle project. For this you would need the following directory structure.
...
TIP: Besides the build.gradle, you can use the exact copy of all files http://www.github.com/tonymkhael/semantic-versioning[in this public repository], as those files do not change much and they only exist to bootstrap your gradle setup.
...
[source, java]
----
include::{assetDir}/SemanticVersioning.java[]
----
...
.build.gradle
----
include::{assetDir}/build.gradle[tags=java]
----
...
To build the project, you have to invoke the gradle **build** task using
...
TIP: Instead of trying to build your .gitignore from scratch, you can use https://github.com/Netflix/gradle-template/blob/master/.gitignore[Netflix's template] which covers pretty much everything for a standard java project.

If you need to look at the available tasks on your project you can use the **tasks** task.
...
Nebula consists of a set of plugins that are opinionated with the Netflix style, but should work really well for you when you follow this approach.

[source, groovy]
.build.gradle
----
include::{assetDir}/build.gradle[tags=nebula-project]
----
...
.build.gradle
----
include::{assetDir}/build.gradle[tags=asciidoctor]
----
...
.build.gradle
----
include::{assetDir}/build.gradle[tags=publishing]
----
...
.build.gradle
----
include::{assetDir}/build.gradle[tags=nebula-release]
----
...
In general, you will need 2 jobs, one for building the project and another to handle the release.
...
Here are the full contents of the build.gradle used.

[source, groovy]
.build.gradle
----
include::{assetDir}/build.gradle[]
----

You can find the full sources http://www.github.com/tonymkhael/semantic-versioning[in this public repository].
Thank you for making it this far, and hope this helps you in some way!

== Useful links

* Semantic versioning
** The standard: http://semver.org
** GitFlow & the branching model : http://nvie.com/posts/a-successful-git-branching-model
* Neflix nebula
** Blog: https://nebula-plugins.github.io
** Sources: https://github.com/nebula-plugins
* Gradle
** Site: http://www.gradle.org
** Differences with maven: https://gradle.org/maven_vs_gradle
